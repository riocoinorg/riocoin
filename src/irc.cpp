...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RioCoinTEST2\r");
            Send(hSocket, "WHO #RioCoinTEST2\r");
        } else {
            // randomly join #RioCoin00-#RioCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #RioCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RioCoin%02d\r", channel_number).c_str());
        }
