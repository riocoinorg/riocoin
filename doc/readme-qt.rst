RioCoin-qt: Qt4 GUI for RioCoin
===============================
...
Alternatively, install Qt Creator and open the `RioCoin-qt.pro` file.

An executable named `RioCoin-qt` will be built.
...
A warning for people using the *static binary* version of RioCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of RioCoin is linked against libdb5.3.
...
installed causes RioCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
